...
export const useSiteStore = defineStore("site", {
  ...
  actions: {
    tapBpm() {
      this.beatMatch.tapListeners.forEach(listener =>
        listener.onTap?.(),
      );
      ...
      const lastTap =
        this.beatMatch.taps[this.beatMatch.taps.length - 1];
    ...
    removeBpmListeners(consumer: string) {
      this.beatMatch.tapListeners =
        this.beatMatch.tapListeners.filter(
