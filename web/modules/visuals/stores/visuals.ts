...
import { Octahedron, type GeometryAttributes, mapGeometryAttributes } from "@visuals/util/geometry";
...
export const useVisualsStore = defineStore("visuals", {
  ...
  actions: {
    ...
    randomise() {
      ...
      this.beatMatch.syncToBar = this.beatMatch.enabled ? getRandomBool() : false;
    ...
    movementTick({
      ...
      const bounceTarget = this.getNextBounceTarget(viewportBounds, startingPosition);
    ...
    getNextBounceTarget(
      ...
      if (!this.edgeBounce.target) {
        this.edgeBounce.target = startingPosition?.clone() ?? new Vector3(0, 0, 0);
      }
      ...
      const nextX = this.edgeBounce.target.x + this.edgeBounce.velocity.x * this.edgeBounce.speed;
      const nextY = this.edgeBounce.target.y + this.edgeBounce.velocity.y * this.edgeBounce.speed;
      ...
      const targetX = this.edgeBounce.target.x + this.edgeBounce.velocity.x * this.edgeBounce.speed;
      const targetY = this.edgeBounce.target.y + this.edgeBounce.velocity.y * this.edgeBounce.speed;
    ...
    beatMatchTick(force = false) {
      ...
      if (!force) {
        ...
        if (new Date().getTime() <= siteStore.beatMatch.lastTime.getTime() + intervalMs) {
          return;
        }
      }
    ...
    setBeatMatchEnabled(enabled: boolean | null) {
      ...
      if (!enabled && ![AutoZoomMode.DISABLED, AutoZoomMode.SMOOTH].includes(this.autoZoom.mode)) {
