...
import { mapGeometryAttributes } from "./geometry";
import type { GeometryAttributes, Geometry } from "./geometry";
...
export class Renderer {
  ...
  public setOnMouseMove(onMouseMove: (renderer: this, event: MouseEvent) => void): this {
    this.onMouseMove = onMouseMove;
    return this;
  }

  public setOnClick(onClick: (renderer: this, event: MouseEvent) => void): this {
    this.onClick = onClick;
    return this;
  }

  public setOnScroll(onScroll: (renderer: this, event: WheelEvent) => void): this {
    this.onScroll = onScroll;
    return this;
  }

  public setOnKeyDown(onKeyDown: (renderer: this, event: KeyboardEvent) => void): this {
  ...
  public setGetDefaultGeometry(getDefaultGeometry: () => GeometryAttributes[]): this {
  ...
  public initialise(): this {
    ...
    this.camera = new THREE.PerspectiveCamera(50, this.getWidth() / this.getHeight());
  ...
  protected initialiseEventListeners(): void {
    ...
    document.addEventListener("keydown", event => this.onKeyDown(this, event), false);
    document.addEventListener("wheel", event => this.onScroll(this, event), false);
  }
  ...
  public randomiseRotations() {
    this.geometry!.forEach(geometry => {
      geometry.setRotation(getRandomInt(0, 25), getRandomInt(0, 25), getRandomInt(0, 25));
    });
  }
  ...
  protected getViewportBounds() {
    ...
    const bottomRight = this.getObjectTargetPositionVector(this.getWidth(), this.getHeight());
