...
export abstract class Geometry {
  ...
  public moveTowardPosition(targetPosition: THREE.Vector3, maintainVelocity = false) {
    if (!maintainVelocity) {
  ...
  public getMaterials(): THREE.MeshBasicMaterial[] | THREE.LineBasicMaterial[] {
    return this.object.material instanceof THREE.Material
      ...
      : (this.object.material as THREE.MeshBasicMaterial[] | THREE.LineBasicMaterial[]);
  }
...
export const geometryClasses = [Cube, Sphere, PartialSphere, Octahedron, Dodecahedron, TorusKnot];

const getGeometryClassFromName = (name: string) => {
  const geometryClass = geometryClasses.find(geometryClass => geometryClass.getName() === name);
  if (!geometryClass) {
...
export const mapGeometryAttributes = (geometryAttributes: GeometryAttributes) => {
