...
test.describe("NetworkStatusPage", () => {
  ...
  test("can see connected network status", async ({ page }, testInfo) => {
    ...
    await expect(page.locator(".networkstatus")).toContainText("Connected To Local Network");
    await expect(page.locator(".networkstatus")).not.toContainText(
      "Not Connected To Local Network",
    );
    await expect(page.locator(".networkstatus")).toContainText("Home IP: 250.80.20.30");
    await expect(page.locator(".networkstatus")).toContainText("Your IP: 250.80.20.30");
  ...
  test("can see not connected network status", async ({ page }, testInfo) => {
    ...
    await page.goto("/status");
    await page.waitForTimeout(3000);
    await expect(page.locator(".networkstatus")).toContainText("Not Connected To Local Network");
