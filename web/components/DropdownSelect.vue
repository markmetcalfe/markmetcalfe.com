<template>
  <div class="dropdownselect" :class="{ disabled: disabled }">
    <label v-if="label" :id="labelId" class="dropdownselect-label" :for="id">{{ label }}</label>
    <div
      :id="id"
...
watch(
  ...
  newValue => {
    selectedOption.value =
      props.options.find(option => option.value === newValue) ?? props.options[0] ?? null;
  },
  { immediate: true },
);
...
watch(
  ...
  newOptions => {
    if (selectedOption.value) {
      selectedOption.value =
        newOptions.find(option => option.value === selectedOption.value?.value) ??
        newOptions[0] ??
        null;
    } else {
