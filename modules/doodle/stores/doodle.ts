...
type ServerMessage =
  ...
  | { type: 'round_start', drawer: Player, hint: string, timeLeft: number, round: number, totalRounds: number }
  ...
  actions: {
    ...
    _handleMessage(msg: ServerMessage) {
      switch (msg.type) {
        case 'you_are':
          this.myId = msg.id
          if (this.myName) this.send({ type: 'join', name: this.myName })
          break
        ...
        case 'draw':
          _onDraw?.(msg.event)
          if (msg.event.kind !== 'end') this.drawHistory.push(msg.event)
          break
